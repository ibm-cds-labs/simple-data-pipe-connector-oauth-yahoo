...
function oAuthYahooConnector(){
	...
	this.getYahooDataSetList = function(pipe, done){
		...
		request.get(requestOptions, function(err, response, body) {
	    		...
	    		if(err) {
	    			...
	    			globalLog.error('Call to ' + requestOptions.url + ' failed: ' + err);	  
	    			globalLog.error(' FFDC: ' + util.inspect(requestOptions, 3));	  
					return done('Yahoo API call failed: ' + err, null); 
	    		}

	    		// Yahoo returned an error 
	    		if(response.statusCode > 300) {
	    			var responseBody = JSON.parse(body); 
	    			globalLog.error('Yahoo API call returned status code ' + response.statusCode);	
	    			globalLog.error(' FFDC: ' + util.inspect(body,3));	
					return done(responseBody.error.description, null);	
	    		}
	...
	*/
	...
	this.fetchRecords = function( dataSet, pushRecordFn, done, pipeRunStep, pipeRunStats, pipeRunLog, pipe, pipeRunner ){
		...
		// The log file is attached to the pipe run document, which is stored in the Cloudant repository database named pipe_db.
		// To enable debug logging, set environment variable DEBUG to '*' or to 'sdp-pipe-run' (without the quotes).
		pipeRunLog.info('Data pipe ' + pipe._id + ' is fetching comments for data set ' + dataSet.name + ' from yahoo.');
